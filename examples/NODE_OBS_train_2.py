...
RUN_DESC = "NODE_OBS_limit2"  # For WandB and run dir
...
SEARCH_SPACE = dict(
    ...
    env_task=dict(
        ...
        limits=tune.choice([[-2,2]]),   # DIFFERENT FROM NBFF - this is the average number of clicks per trial
        # had n here before
    ),
    # simulation task params
    env_sim=dict(
        # Task Environment Parameters
        limits=tune.choice([[-2,2]]),   # avg clicks per trial
    ),
)
