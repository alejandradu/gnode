...
RUN_DESC = "NODE_OBS_limit4"  # For WandB and run dir
...
SEARCH_SPACE = dict(
    ...
    env_task=dict(
        ...
        limits=tune.choice([[-4,4]]),   # DIFFERENT FROM NBFF - this is the average number of clicks per trial
        # had n here before
    ),
    # simulation task params
    env_sim=dict(
        # Task Environment Parameters
        limits=tune.choice([[-4,4]]),   # avg clicks per trial
    ),
)
