...
RUN_DESC = "GRU_OBS_300epoch"  # For WandB and run dir
...
SEARCH_SPACE = dict(
    ...
    task_wrapper=dict(
        ...
        weight_decay=tune.grid_search([1e-3, 1e-7]),
        learning_rate=tune.grid_search([1e-4, 1e-6, 1e-7]),
    ),
    trainer=dict(
        # Trainer Parameters 
    ...
    params=dict(
        ...
        # any specific reason why these are a lot bigger than from node?
    ...
    #     n_timesteps=tune.choice([500]), 
    #     noise=tune.choice([0.15]),
    #     switch_prob=tune.choice([0.05]),
    #     transition_blind=tune.choice([1]),
    # ),
...
#
# could also add in the search space: datamodule_task, datamodule_sim, simulator, callbacks, loggers
#
