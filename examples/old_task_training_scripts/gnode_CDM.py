...
RUN_DESC = "gnode_cdm_test_par"  # For WandB and run dir
...
def main(
    ...
    scaling_config = ScalingConfig(    
        ...
        num_workers=4,  # number of distributed workers - should match total trials?
        use_gpu=True,    # whether each worker should use a gpu
        resources_per_worker={"CPU": 1, "GPU": 0.25},   # default, but consider cluster architecture to change
    ...
    if DISTRIBUTED_TRAINING:
        ...
        ray_trainable = tune.with_resources(trainable, {'cpu': 1, 'gpu': 1})
    ...
    tuner_object = tune.Tuner(
        ...
        tune_config=tune.TuneConfig(
            metric="loss",
            mode="min",
            num_samples=4,    # TODO: should this be more?
...
# TODO: difference training encoding and decoding??