...
def train_parallel(   
    ...
    trainer: pl.Trainer = hydra.utils.instantiate(
        config_all["trainer"],
        logger=logger,
        callbacks=callbacks.append(rtl.RayTrainReportCallback()),   # keep monitoring progress in parallel
        #callbacks=[rtl.RayTrainReportCallback()]
        #I've seen this happen when lightning.pytorch and pytorch_lightning imports are mixed together. 
        # You need to use only one of them. This includes the callback parent
        accelerator="auto",
        _convert_="all",
        devices="auto",
        strategy = rtl.RayDDPStrategy(),     # distributed data parallel strategy # TODO: change this if I dont want to distribute?? 
